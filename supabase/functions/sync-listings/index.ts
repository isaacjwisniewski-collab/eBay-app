...
Deno.serve(async (req: Request) => {
  ...
  const { data: accounts } = await supabase
    .from("ebay_accounts").select("*").eq("is_active", true).limit(1);

  if (!accounts || accounts.length === 0) {
    return new Response(JSON.stringify({ error: "No connected eBay accounts" }), { status: 400, headers: corsHeaders });
  }
  ...
  try {
    ...
    const stats = {
      avg: prices.length > 0 ? Math.round((prices.reduce((a: number, b: number) => a + b, 0) / prices.length) * 100) / 100 : 0,
